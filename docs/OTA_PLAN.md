...
- First install (blank device, or one on a pre-1.6.0 bootloader) comes from
  `docs/flash.html`: esp-web-tools (vendored in `docs/vendor/`) writes
  `docs/firmware/airtrack-<version>-factory.bin` at offset 0 over Web Serial.
  The factory image and its `web-flash.json` are regenerated by
  `tools/publish_release.sh` and must be served from the Pages origin, because
  GitHub release assets carry no `Access-Control-Allow-Origin` header.
